...
import com.rx.schedulers.RxIOScheduler;
import com.rx.schedulers.RxSingleScheduler;
...
class RxObservableTest {
    ...
    void testCreateAndSubscribe() {
        ...
        source.subscribe(received::add, Throwable::printStackTrace, () -> received.add("DONE"));
    ...
    void testSubscribeOnScheduler() throws InterruptedException {
        ...
        RxObservable.just("X").subscribeOn(new RxIOScheduler()).subscribe(item -> {
    ...
    void testObserveOnScheduler() throws InterruptedException {
        ...
        RxObservable.just("Y").observeOn(new RxSingleScheduler()).subscribe(item -> {
            threadName.set(Thread.currentThread().getName());
            latch.countDown();
        });
